...
システムは保存されているジョブの一覧を取得できなければならない（SHALL）。一覧には各ジョブの ID、ステータス、種別、通知チャネル、作成日時が含まれる。
...
システムは指定された JobId のジョブ詳細を取得できなければならない（SHALL）。詳細には ID、ステータス、種別、通知チャネル、作成日時、開始日時、完了日時、実行結果が含まれる。
